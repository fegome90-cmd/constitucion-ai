...
Esta sección establece las definiciones base de la constitución. Para términos adicionales y definiciones interpretativas detalladas, véase el Glosario al final de este documento.
...
Toda tarea relevante de implementación, corrección o refactor deberá realizarse, cuando corresponda, en un worktree aislado.
...
La validación deberá escalar según el tipo de cambio, su riesgo y la superficie afectada, pudiendo incluir pruebas unitarias, de integración, smoke, end-to-end o revisión manual justificada cuando la automatización no sea viable o suficiente.
...
- La revisión manual solo será suficiente cuando esté justificada por el tipo de cambio y acompañada de evidencia trazable.
...
El sistema deberá mantener, cuando corresponda, una fuente de verdad para el contexto y otra para el trabajo comprometido.
...
- Sí deberán documentarse, al menos en el punto correcto, los cambios de contrato, comandos, configuración, flujos críticos, decisiones arquitectónicas relevantes y comportamientos visibles.
...
El sistema deberá diseñarse, en la medida razonable, de forma agnóstico respecto del proveedor, familia o versión de modelo.
...
Solo podrá optimizarse una parte del sistema para un modelo específico si el beneficio técnico es real y el acoplamiento queda documentado, acotado y reversible.
...
Toda acción destructiva, irreversible o de alto impacto deberá requerir validación reforzada y, cuando corresponda, aprobación humana o control compensatorio equivalente.
...
Toda ejecución con capacidad de mutación, acceso externo o interacción con recursos críticos deberá realizarse en un entorno acotado, aislado o controlado cuando la naturaleza del trabajo lo requiera.
...
#### 14.4. Autoridades de aprobación

El sistema deberá definir explícitamente:

- Quiénes están autorizados a conceder aprobaciones (personas, roles o funciones específicas).
- Niveles de autoridad según tipo de acción (ej: aprobador técnico vs aprobador de negocio vs aprobador de seguridad).
- Mecanismo de registro de aprobaciones (log, sistema de tickets, documento de registro).
...
Las reglas críticas de integridad deberán vivir, cuando corresponda, también en la base de datos y no solo en la aplicación.
...
El sistema no deberá acoplar su dominio innecesariamente a peculiaridades de un motor específico de base de datos, salvo que exista beneficio técnico explícito y el costo quede asumido.
Toda dependencia fuerte de features particulares deberá documentarse como decisión deliberada.
...
El contrato deberá definir, cuando corresponda, entradas, salidas, efectos laterales, invariantes, límites operativos, errores esperables, formato, compatibilidad y condiciones de uso.
...
- Trabajar en rama identificable y, cuando corresponda, en worktree aislado.
...
### A. Validación automática (CI/CD, hooks, tooling)
...
Esta versión 0.7 sanea, endurece y consolida la constitución mediante:

**Cambios estructurales:**

- Fusión de secciones de definiciones: "Definiciones Fundamentales" (7 términos) + referencia al Glosario completo.
- Eliminación de redundancia entre "Definiciones" y "Glosario y Definiciones Interpretativas".

**Cambios normativos:**

- **Art. 14 en Ley VIII:** Clasificación explícita de acciones con aprobación humana obligatoria vs condicional, con autoridades de aprobación y requisitos de registro.
- **Régimen de excepciones (Art. 4):** Agregada autoridad de aprobación por nivel de riesgo, mecanismo de registro, y duración máxima por defecto (30 días).
- **Régimen de excepciones (Art. 7-8):** Agregado límite de repetición (3 veces → evaluar conversión en regla) y mecanismos de cierre automático/manual.

**Endurecimientos de gobernanza:**

- Aprobación humana obligatoria para: producción/despliegue, secretos/credenciales, borrados masivos, permisos/gobernanza.
- Autoridades de aprobación definidas por nivel de riesgo.
- Registro obligatorio de aprobaciones y excepciones.
- Cierre automático de excepciones por vencimiento o condición.

**Redundancias eliminadas:**

- Definiciones duplicadas de mutación, tarea, validación, evidencia, fuente de verdad, excepción, riesgo.
- Errores de formato corregidos (espacios faltantes, saltos de línea).

**Nuevo anexo:**

- **Anexo II: Mapa de enforcement constitucional** - Mapea cómo se verifica cada familia de normas (validación automática, revisión humana, evidencia documental, auditoría posterior) con tabla por familia de normas y responsabilidades de enforcement.

Las futuras versiones podrán ampliar artículos, incisos y casos de aplicación, pero no deberán inflar el número de leyes sin necesidad clara.

---

# Glosario y Definiciones Interpretativas

Estas definiciones fijan el sentido oficial de los términos usados en esta constitución.
Su función es reducir ambigüedad, evitar interpretaciones oportunistas y mantener consistencia entre diseño, ejecución, validación y cierre.
